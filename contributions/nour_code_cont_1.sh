...
First of all, I worked on a dataset SRR8115017.fastq, I have decided to work with bowtie2 for alignment because I read their documentation I found their indexing is more efficient than BWA in addition to it’s manual is more readable and informative, so before alignment step I made me mind to read the whole manual  and understand almost their arguments.
...
Before beginning in the alignment step, I remember from the assignment that when I run the haplotypecaller, It has an error that due to the sample name and something like that, so I decided to include in the SAM file the header informations that includes the read group, platform unit, library preparation and sample name, I know that I am working with just one sample so it has the same library preparation in addition to other parameters but I feel that their absence is the cause of haplotypecaller error.
...
@RG	ID:@SRR8115017	SM=SRR8115017	PL=Illumina	LB=SRR8115017_same	PU=@SRR8115017.; File /home/nourelislam/BaseRecalibration_Benchmarking/test_two.bam; Line number 3

So I realized that the error is due to the formatting of the SAM header, and after many trials, I found the error in the “ = ” sign, so I’ve changed it to “ : ” and repeat the alignment step and the Markduplicate step then the runnin is going smoothly.
...
Error A USER ERROR has occurred: Error while trying to create index for /home/nourelislam/BaseRecalibration_Benchmarking/Homo_sapiens_chr21.vcf. Error was: htsjdk.tribble.TribbleException: The provided VCF file is malformed at approximately line number 1291601: empty alleles are not permitted in VCF records

After searching about this error which is a normal error in the vcf file, some suggested to completely remove the record having that malformation, so when I run the following code and remove the specific malformed record, it runs smoothly.
