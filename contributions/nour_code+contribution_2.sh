...
After I noticed that there is no enough variants in the gvcf file that was created in the 1st pipeline, there was roughly one page of variants in the file, I thought that all of that due to the bad mapping quality with chr21 which was 33.83%, hence the mapping 33% so the variants in those mapped reads presents in small extent. In other words, there is no enough variants in those low mapped reads to generate a large variants in vcf file.
